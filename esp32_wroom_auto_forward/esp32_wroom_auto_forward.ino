...
// ==== PWM/LEDC ====
const int LEDC_HZ   = 50; // 50 Hz for (continuous) servo signal
const int LEDC_BITS = 16;
const uint32_t TOP  = (1UL << LEDC_BITS) - 1;
const uint32_t PERIOD_US = 20000;

// Wheels: LF, RF, LR, RR -> pins
const int PIN_LF = 23;
const int PIN_RF = 22;
const int PIN_LR = 19;
const int PIN_RR = 18;
const int WHEEL_PINS[4] = { PIN_LF, PIN_RF, PIN_LR, PIN_RR };
...
// int TRIM_STOP_US[4]={275,160,175,84};
// int TRIM_STOP_US[4]={170,160,175,173};
int TRIM_STOP_US[4]={301,160,175,173};
int TRIM_FWD_US [4]={46,40,42,32};
...
void forwardAll(){
  // neurtal_us (microsecond) + offset for each wheel + drection * (base + offset)
...
pre{background:#f4f4f4;padding:8px;white-space:pre-wrap;text-align:left}</style>
<h1>Andechser-1</h1>
<div><button onclick="fetch('/forward').then(()=>update())">Forward</button></div>
<div><button onclick="fetch('/stop').then(()=>update())">Stop</button></div>
...
void handleForward(){ forwardAll(); server.send(200,"text/plain","OK"); }
